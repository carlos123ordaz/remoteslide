    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: BG, color: FG }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {user ? (
            ...
            <button onClick={handleGoogleLogin} style={btnGhost}>Iniciar sesión</button>
          )}
          <button onClick={() => navigate('/upload')} style={{ ...btnPrimary }}>Empezar</button>
        </div>
      </header>
      ...
      <main
        style={{
          ...
          gap: 64,
          padding: '88px max(64px, 8vw) 64px',
          alignItems: 'center',
          maxWidth: 1440,
          margin: '0 auto',
          width: '100%',
        ...
        <div>
          <div
            style={{
              ...
              fontSize: 12, color: MUTED, marginBottom: 32,
          ...
          <p style={{ fontSize: 18, color: MUTED, lineHeight: 1.5, marginTop: 24, maxWidth: 480 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 32 }}>
            <button
              ...
              style={{ ...btnPrimary, height: 46, padding: '0 22px', fontSize: 15 }}
            >
              Subir presentación →
            </button>
            {!user && (
              <button
                ...
                style={{ ...btnGhost, height: 46, padding: '0 18px', fontSize: 15, display: 'inline-flex', alignItems: 'center', gap: 8 }}
              >
                <GoogleIcon />
                Entrar con Google
              </button>
      ...
      <section style={{ borderTop: '1px solid ' + BORDER, padding: '24px max(64px, 8vw)' }}>
        <div
          style={{
            ...
            fontFamily: "'Geist Mono', monospace",
            fontSize: 11, letterSpacing: '1.4px',
            textTransform: 'uppercase', color: '#9A9A9A',
          }}
        >
          <span>Funciona en sesiones de</span>
          <span>Teams</span><span>Zoom</span><span>Meet</span><span>Webex</span><span>Anywhere</span>
        </div>
      </section>

      {/* How it works */}
      <section style={{ background: '#fff', borderTop: '1px solid ' + BORDER, padding: '80px max(64px, 8vw)' }}>
        <div style={{ maxWidth: 1440, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(32px, 3.4vw, 44px)', fontWeight: 600, letterSpacing: '-0.025em', margin: '0 0 48px', maxWidth: 720, lineHeight: 1.05 }}>
            Tres pasos. Sin descargas, sin cables, sin Bluetooth.
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32 }}>
            {[
              { n: '01', t: 'Subí tu archivo', d: 'PPTX o PDF. Lo procesamos y generamos un código QR único para tu sesión.' },
              ...
              <div key={i}>
                <div style={{ fontFamily: "'Geist Mono', monospace", fontSize: 12, color: '#9A9A9A', marginBottom: 16 }}>{f.n}</div>
                <div style={{ fontSize: 20, fontWeight: 600, letterSpacing: '-0.015em', marginBottom: 8 }}>{f.t}</div>
      ...
      {/* Auth CTA */}
      {!user && (
        <section style={{ padding: '64px max(64px, 8vw)', borderTop: '1px solid ' + BORDER }}>
          <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h2 style={{ fontSize: 28, fontWeight: 600, letterSpacing: '-0.02em', margin: '0 0 8px' }}>
                Guardá tus presentaciones para siempre.
              </h2>
              <p style={{ fontSize: 15, color: MUTED, margin: 0 }}>
                Sin cuenta, cada sesión dura 24 horas. Con cuenta Google, tus presentaciones se guardan indefinidamente.
              </p>
            </div>
            <button
              onClick={handleGoogleLogin}
              style={{ ...btnPrimary, height: 46, padding: '0 22px', fontSize: 15, flexShrink: 0, marginLeft: 40, display: 'inline-flex', alignItems: 'center', gap: 10 }}
            >
      ...
      >
        ...
        <span style={{ fontFamily: "'Geist Mono', monospace", letterSpacing: '0.6px' }}>v1.0 · estable</span>
      </footer>
    </div>
  )
}
...
function HeroVisual() {
  ...
  return (
    <div style={{ position: 'relative', aspectRatio: '1/0.85', minHeight: 420 }}>
      ...
      <div style={{ position: 'absolute', left: '0%', right: '8%', top: 'calc(78% + 0px)', height: 12, background: FG, borderRadius: '0 0 10px 10px' }} />
      ...
      <div style={{ position: 'absolute', bottom: '0%', right: '0%', width: '36%', aspectRatio: '0.48/1', background: FG, borderRadius: 28, padding: 8, boxShadow: '0 28px 50px -10px rgba(0,0,0,0.32)' }}>
        <div style={{ width: '100%', height: '100%', background: '#fff', borderRadius: 22, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ margin: '8px 10px 0', aspectRatio: '16/10', background: BG, borderRadius: 4, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <MockSlide small />
          </div>
          <div style={{ flex: 1 }} />
...
function MockSlide({ small }) {
  return (
    <div style={{ width: '100%', height: '100%', background: '#0A0A0A', display: 'flex', flexDirection: 'column', padding: small ? '8%' : '7%', justifyContent: 'space-between' }}>
