...
export default function Dashboard({ user }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, color: FG }}>
      {/* Header */}
      <header style={{ height: 60, display: 'flex', alignItems: 'center', padding: '0 32px', borderBottom: '1px solid ' + BORDER, background: 'rgba(250,250,247,0.85)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 50 }}>
        <Wordmark size={17} />
        <div style={{ flex: 1 }} />
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 48 }}>
          <div>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 600, letterSpacing: '-0.025em', margin: 0 }}>
              Hola, {user.user_metadata?.given_name || 'presenter'}.
            </h1>
          </div>
          <button
            onClick={() => navigate('/upload')}
            style={{ height: 42, padding: '0 20px', borderRadius: 6, background: FG, color: '#fff', border: 'none', fontFamily: 'inherit', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}
          >
            Nueva presentación →
          </button>
        </div>
          ...
          <>
            ...
            <section style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <h2 style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.015em', margin: 0 }}>Mis presentaciones</h2>
                <div style={{ fontFamily: "'Geist Mono', monospace", fontSize: 11, color: MUTED }}>{presentations.length} archivos</div>
              </div>

              {presentations.length === 0 ? (
                <div style={{ background: '#fff', border: '1px solid ' + BORDER, borderRadius: 8, padding: '48px 32px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                  {presentations.map((p) => (
                    <div
                      key={p.id}
                      style={{ background: '#fff', border: '1px solid ' + BORDER, borderRadius: 8, padding: 20, cursor: 'pointer', transition: 'border-color 0.15s', position: 'relative' }}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                        <button
                          onClick={() => {
                            const { data } = supabase.storage.from('presentations').getPublicUrl(p.file_path)
                            navigate('/upload')
                          }}
                          style={{ flex: 1, height: 32, borderRadius: 6, background: BG, color: FG, border: '1px solid ' + BORDER, fontFamily: 'inherit', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}
                        >
                          Presentar de nuevo
                        </button>
                      </div>
            ...
            {sessions.length > 0 && (
              <section>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <h2 style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.015em', margin: 0 }}>Sesiones recientes</h2>
                </div>
                <div style={{ background: '#fff', border: '1px solid ' + BORDER, borderRadius: 8, overflow: 'hidden' }}>
                  {sessions.map((s, i) => (
                    <div
                      key={s.id}
                      style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', borderTop: i ? '1px solid ' + BORDER : 'none' }}
                    >
                      ...
                      {s.is_active && (
                        <button
                          ...
                          style={{ height: 30, padding: '0 12px', borderRadius: 6, background: FG, color: '#fff', border: 'none', fontFamily: 'inherit', fontSize: 12, fontWeight: 500, cursor: 'pointer', flexShrink: 0 }}
                        >
                          Retomar
                        </button>
                      )}
